...
class RedisMetadataFieldsPluginExtension extends BaseExtension
{
    ...
    {
        $storageHost = Env::get('REDIS_METADATA_FIELDS_HOST', $config['host']);
        if (null === $storageHost) {
        ...
        $storagePort = Env::get('REDIS_METADATA_FIELDS_PORT', $config['port']);
        if (null === $storageHost) {
        ...
        return [
            ...
            'apisearch_plugin.redis_metadata_fields.is_cluster' => (bool) Env::get('REDIS_METADATA_FIELDS_IS_CLUSTER', $config['is_cluster']),
            'apisearch_plugin.redis_metadata_fields.database' => (string) Env::get('REDIS_METADATA_FIELDS_DATABASE', $config['database']),
            'apisearch_plugin.redis_metadata_fields.key' => (string) Env::get('REDIS_METADATA_FIELDS_KEY', $config['key']),
        ];
    }
}
